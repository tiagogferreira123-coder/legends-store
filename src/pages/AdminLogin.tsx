...
export const AdminLogin = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // A chave padrão é "admin123" (podes mudar no StoreContext se quiseres)
    if (loginAdmin(key)) {
      ...
      <div className="max-w-md w-full bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <input
            ...
            className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500 text-center tracking-widest"
